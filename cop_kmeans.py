...
def cop_kmeans(dataset, k, ml=[], cl=[],
    ...
    for _ in range(max_iter):
        clusters_ = [-1] * len(dataset)     # 使用 -1 标记数据点未被分配
        for i, d in enumerate(dataset):
            ...
            if clusters_[i] == -1:
                ...
                while (not found_cluster) and counter < len(indices):
                    index = indices[counter]        # 当前微簇中心的索引
...
def cop_kmeans(x, y, n_clusters, init="k-means++", max_iter=300, tol=1e-4):
    tol = tolerance(tol, x)

    # 中心点坐标
    centers = initialize_centers(x, n_clusters, init)

    for _ in range(max_iter):
        point = [-1] * x.shape[0]
        for data_index, data in enumerate(x):
            # 中心点centers的索引和距离列表
            indices, _ = closest_clusters(centers, data)
            counter = 0

            if point[data_index] == -1:
                found_cluster = False

                while(not found_cluster) and (counter < len(indices)):
                    center_index = indices[counter]
    pass
...
def violate_constraints(data_index, center_index, clusters, y):
    if (clusters[data_index]!=-1 and clusters[data_index]!=cluster_index) or (y[data_index]!=y[cluster_index]):
        return True
